...
def apply_alpha(colors: ArrayLike, alpha: int):
    arr = np.asarray(colors).copy()
    alpha = int(np.clip(alpha, 0, 255))
    if arr.ndim == 1:
        if arr.shape[0] == 3:
            arr = np.concatenate([arr, np.array([alpha], dtype=arr.dtype)])
        elif arr.shape[0] >= 4:
            arr[3] = alpha
        return arr

    if arr.shape[-1] == 3:
        alpha_channel = np.full(arr.shape[:-1] + (1,), alpha, dtype=arr.dtype)
...
def normalize_alpha(alpha) -> int:
    alpha_value = float(alpha)
    if alpha_value <= 1.0:
        alpha_value *= 255.0
    return int(np.clip(round(alpha_value), 0, 255))
...
class MeshNode(trimesh.Trimesh):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._color = color
        self._opacity = 255 if len(color) < 4 else normalize_alpha(color[3])
        self._name = 'mesh'
    ...
    def update_node(self):
        ...
        old_node = getattr(self, "nodes", None)
        old_visible = bool(getattr(old_node, "visible", True))
        if old_node is not None and hasattr(old_node, "remove"):
            old_node.remove()
        ...
        if hasattr(self.nodes, "visible"):
            self.nodes.visible = old_visible
    ...
    def opacity(self, opacity):
        self._opacity = normalize_alpha(opacity)
        self._set_color = False
        self.update_node()
...
class SurfaceNode(MeshNode):
    ...
    def set_colors(self):
        ...
        if self.colored_by == 'value':
            ...
            colors = apply_alpha(colors, self._opacity)
            ...
            self.visual = TextureVisuals(
                ...
                PBRMaterial(
                    roughnessFactor=0.4,
                    baseColorFactor=[255, 255, 255, self._opacity],
                    metallicFactor=.4,
                    baseColorTexture=img,
                    doubleSided=True,
                    alphaMode='OPAQUE' if self._opacity >= 255 else 'BLEND',
                ))
            return

        elif self.colored_by == 'vertex':
            self.visual.vertex_colors = apply_alpha(self._vertex_colors, self._opacity)
        elif self.colored_by == 'face':
            self.visual.face_colors = apply_alpha(self._face_colors, self._opacity)
        elif self.colored_by == 'uniform':
            self.visual.vertex_colors = np.tile(
                apply_alpha(self._color, self._opacity),
    ...
    def _set_visual(self):
        ...
        self.visual.material.baseColorFactor = [255, 255, 255, self._opacity]
        self.visual.material.alphaMode = 'OPAQUE' if self._opacity >= 255 else 'BLEND'
